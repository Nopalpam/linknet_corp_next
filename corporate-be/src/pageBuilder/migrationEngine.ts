...
function splitPath(path: string): string[] {
  ...
  return parts.every(isSafeObjectKey) ? parts : [];
}
...
function deepMergePreservingCustom(defaults: any, value: any): any {
  ...
  if (isRecord(defaults)) {
    const mergedEntries = Object.entries(deepClone(defaults));
    if (!isRecord(value)) {
      return Object.assign(Object.create(null), Object.fromEntries(mergedEntries));
    }
    ...
    for (const [key, nextValue] of Object.entries(value)) {
      ...
      mergedEntries.push([key, key in defaults
...
function ensureConfig(data: Record<string, any>): Record<string, any> {
  if (!isRecord(data.config)) data.config = Object.create(null);
  return data.config;
}
...
function ensureIntro(data: Record<string, any>): Record<string, any> {
  ...
  if (!isRecord(intro)) {
    data.introData = Object.create(null);
  } else {
...
function normalizeHero(data: Record<string, any>): void {
  ...
  for (const key of [
    'title', 'description', 'pill_text', 'labelText',
    'button_text', 'button_link', 'button_label', 'button_url',
    'cta_text', 'cta_link', 'cta_label', 'cta_url',
  ]) {
    delete data[key];
  }
}
