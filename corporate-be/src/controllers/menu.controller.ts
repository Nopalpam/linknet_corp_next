...
function normalizeMenuTranslations(translations: any): Record<string, any> | null | undefined {
  ...
  return Object.entries(translations).reduce((normalized, [locale, value]) => {
    if (typeof value === 'string') {
      normalized[locale] = { title: value };
      return normalized;
    }
...
export class MenuController {
  ...
  async createMenu(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const normalizedTranslations = normalizeMenuTranslations(translations);
