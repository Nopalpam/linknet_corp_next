...
export class UserService {
  ...
  async createUser(dto: CreateUserDto, createdBy: string): Promise<UserDetailResponse> {
    ...
    if (existingUsername) {
      throw new AppError('Username already exists', 409, 'DUPLICATE_ENTRY');
    }
  ...
  async updateUser(
    ...
    if (dto.username && dto.username !== user.username) changes.push('username');
  ...
  async deleteUser(userId: string, deletedBy: string): Promise<void> {
    ...
    const hasSuperAdminRole = user.userRoles.some(
      (ur) => ['super-admin', 'super_admin'].includes(ur.role.slug) && ur.role.isSystem
    );
  ...
  async bulkDeleteUsers(dto: BulkDeleteUsersDto, deletedBy: string): Promise<{ deleted: number }> {
    ...
    const superAdminUsers = users.filter((u) =>
      u.userRoles.some((ur) => ['super-admin', 'super_admin'].includes(ur.role.slug) && ur.role.isSystem)
    );
