...
function stripHtmlForReadingTime(value: string): string {
  let output = '';
  let insideTag = false;
...
function serializeNews(item: any) {
  if (!item) return item;
  const readingText = item.content_en ? stripHtmlForReadingTime(item.content_en) : '';
  ...
  return {
    ...
    readingTime: readingText ? Math.max(1, Math.ceil(readingText.split(/\s+/).length / 200)) : 1,
  };
}
