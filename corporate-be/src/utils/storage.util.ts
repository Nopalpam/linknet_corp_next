...
import { normalizeStorageFilename } from './storagePathSecurity.util';
...
const uploadToLocal = async (
  ...
  try {
    ...
    const originalBaseName = path.basename(filename.split('\\').join('/'));
    const uniqueFilename = normalizeStorageFilename(`${uuidv4()}${path.extname(originalBaseName).toLowerCase()}`);
    const avatarDir = path.join(UPLOAD_DIR, 'avatars');
    ...
    logger.info('Uploaded avatar to local storage', {
...
const uploadToAzure = async (
  ...
  try {
    ...
    // Create blob client
    const originalBaseName = path.basename(filename.split('\\').join('/'));
    const blobName = normalizeStorageFilename(`${uuidv4()}${path.extname(originalBaseName).toLowerCase()}`);
...
export const uploadToAzureBlob = async (
  ...
  switch (driver) {
    ...
    case 's3': {
      ...
      logger.info('Uploaded avatar to S3', { key: result.cloudKey });
      return {
        url: result.url,
    ...
    default:
      logger.warn('Unknown storage driver, falling back to local', { driver });
      return uploadToLocal(buffer, filename);
  }
};
...
export const extractBlobNameFromUrl = (url: string): string | null => {
  try {
    ...
    if (!url.includes('/')) {
      return normalizeStorageFilename(url);
    }

    // Extract filename from URL
    const urlParts = url.split('/');
    const filename = urlParts[urlParts.length - 1];
    return filename ? normalizeStorageFilename(filename) : null;
  } catch (error) {
...
export const deleteFromAzureBlob = async (filenameOrUrl: string): Promise<boolean> => {
  ...
  logger.info('Deleting from storage driver', { driver, filename });
