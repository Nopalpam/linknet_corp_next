...
function resolveMediaUrl(value: any): string {
  ...
  for (const key of directKeys) {
    ...
    if (nestedValue && typeof nestedValue === 'object') {
      const nestedUrl = resolveMediaUrl(nestedValue);
      if (nestedUrl) return nestedUrl;
    }
  }

  const nestedContainers = ['asset', 'media', 'fileData', 'upload', 'data'];
  for (const key of nestedContainers) {
    const nestedUrl = resolveMediaUrl(value[key]);
    if (nestedUrl) return nestedUrl;
  }
...
function resolveLinkUrl(value: any): string {
  if (value == null) return '';
  if (typeof value === 'string') return value.trim();
  if (typeof value !== 'object') return String(value).trim();
...
function firstNonEmpty(...values: any[]): string {
  for (const value of values) {
    const resolvedValue = resolveLinkUrl(value);
...
function hasExplicitCtaList(data: Record<string, any>): boolean {
  return ['ctaList', 'cta_list', 'ctaButtons', 'cta_buttons', 'buttons'].some((key) => Array.isArray(data?.[key]));
}
...
function mapAwardItem(award: Record<string, any>, t: (f: any) => string, locale: string) {
  ...
  return {
    ...
    href: firstNonEmpty(award.href, award.url, award.link),
    year: award.year,
    issuer: award.issuer || '',
  };
}
...
export function normalizeComponentData(data: Record<string, any> | undefined): Record<string, any> {
  ...
  const shouldKeepCtaList = ctaList.length > 0 || hasExplicitCtaList(source);
...
function passCmsData(
  ...
  const shouldKeepCtaList = ctaList.length > 0 || hasExplicitCtaList(resolvedData);
...
export const COMPONENT_MAP: Record<string, ComponentMapEntry> = {
  ...
  hero_section: {
    ...
    mapProps: ({ data, t, locale }) => {
      ...
      const hasIntroField = (field: string) => (
        hasExplicitIntro && Object.prototype.hasOwnProperty.call(introSource, field)
      );
  ...
  about_with_marquee: {
    ...
    mapProps: ({ data, t, styleProps, locale }) => ({
      cmsData: {
        id: data.section_id || data.sectionId || data.id || 'about-running-photos',
        config: data.config || {},
        introData: extractIntro(data, t, locale) || {
          as: 'h2',
        ...
        photos: (Array.isArray(data.photos)
          ...
          return {
            url,
            alt: t(photo?.alt || photo?.altImage || photo?.alt_image || photo?.title) || `Linknet Activity ${index + 1}`,
          };
        }).filter(Boolean),
        marqueeSpeed: Number(data.marquee_speed ?? data.marqueeSpeed) || 30,
        marqueeDirection: data.marquee_direction || data.marqueeDirection || 'left',
      },
      ...styleProps,
    }),
  },
  ...
  business_tab: {
    ...
    mapProps: ({ data, t, styleProps, locale }) => {
      ...
      return {
        cmsData: {
          id: data.section_id || data.sectionId || data.id || 'business-tabs',
          config: data.config || {},
          introData: presentation.introData,
  ...
  hero: {
    ...
    mapProps: ({ data, t, locale, styleProps }) => {
      ...
      const ctaList = getRawCtaList(data).map((cta, index) => normalizeCtaItem(cta, t, index));
      const shouldKeepCtaList = ctaList.length > 0 || hasExplicitCtaList(data);
      ...
      return {
        ...
        data: {
          ...
          introData,
          ctaList: shouldKeepCtaList ? ctaList : undefined,
