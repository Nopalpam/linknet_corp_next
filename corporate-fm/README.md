...
```bash
curl -X POST https://dev-be.lncorp.local/api/upload \
...
```bash
curl "https://dev-be.lncorp.local/api/files?prefix=images/&limit=50" \
  -H "x-api-key: your_key"
```
...
```bash
curl -X DELETE "https://dev-be.lncorp.local/api/file?key=images%2Fuuid.jpg" \
  -H "x-api-key: your_key"
```
...
```bash
curl -X DELETE "https://dev-be.lncorp.local/api/files" \
  -H "x-api-key: your_key" \
...
```bash
curl "https://dev-be.lncorp.local/api/signed-url?key=images%2Fuuid.jpg&expires=7200" \
  -H "x-api-key: your_key"
```
...
```bash
curl https://dev-be.lncorp.local/health
```
