...
(() => {
    ...
    // If raw contains HTML tags, return it unchanged. Otherwise convert newlines to paragraphs.
    const formatParagraphs = (raw = "") => {
        ...
        if (!norm) return "";
        return `<p class='mb-3'>${norm.replace(/\n{2,}/g, "</p><p class='mb-3'>").replace(/\n/g, "<br>")}</p>`;
    };
    ...
    const sanitizeHTML = (html = "") => {
        ...
        // Fallback: try a lightweight DOM-based sanitization.
        // First decode any HTML entities (so data-bio which was escaped becomes real HTML)
        const decoder = document.createElement('textarea');
        decoder.innerHTML = html;
        const decoded = decoder.value;

        const wrap = document.createElement('div');
        // Set decoded HTML and then strip dangerous nodes/attributes
        ...
        const walker = wrap.querySelectorAll('*');
        walker.forEach(node => {
            // Clone attribute list to avoid modifying while iterating
            Array.from(node.attributes || []).forEach(attr => {
                const name = attr.name.toLowerCase();
                const val = (attr.value || '').toLowerCase();
                if (name.startsWith('on')) {
                    node.removeAttribute(attr.name);
                } else if (name === 'href' && val.startsWith('javascript:')) {
                    node.removeAttribute('href');
                } else if (name === 'src' && val.startsWith('javascript:')) {
                    node.removeAttribute('src');
                } else if (name === 'style') {
                    // optional: remove inline styles for safety
                    node.removeAttribute('style');
                }
            });
        });

        // Ensure anchor tags open safely
        wrap.querySelectorAll('a[href]').forEach(a => {
            if (!a.hasAttribute('target')) a.setAttribute('target','_blank');
            a.setAttribute('rel','noopener noreferrer');
            // If href is empty after stripping, remove the attribute to avoid empty links
    ...
    const readItem = (el) => {
        const rawBioData = el.dataset.bio || '';
        // Prefer dataset.bio (HTML-escaped), decode it. If not present, use hidden element's innerHTML.
        let bio = '';
        if (rawBioData) {
            bio = decodeHtmlEntities(rawBioData);
        } else {
            const hidden = el.querySelector('.lnManagementCard__bio');
            if (hidden) bio = hidden.innerHTML || hidden.textContent || '';
        }
    ...
    const render = (idx) => {
        ...
        console.log('Rendering modal for:', d);
