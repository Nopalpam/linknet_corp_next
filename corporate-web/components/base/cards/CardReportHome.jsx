...
const DEFAULT_REPORT_ICON_SRC = 'https://dev.linknet.co.id/assets/icons/pdf-circle.svg';
...
function renderCtaIcon(value) {
  if (!value) return undefined;
  if (typeof value !== 'string') return value;

  return isImageSource(value)
    ? <img src={value} alt="" className="h-[1em] w-[1em] object-contain" aria-hidden="true" />
    : <Icon name={value} />;
}
      ...
      <div className="lnCardReportHome__body flex-1 p-8 flex flex-col items-start bg-light-2 relative rounded-[20px]">
        ...
        {label && (
          <LinknetLink 
            ...
            iconLeft={renderCtaIcon(primaryCta?.iconLeft || primaryCta?.icon_left)}
            iconRight={renderCtaIcon(primaryCta?.iconRight || primaryCta?.icon_right || primaryCta?.icon)}
