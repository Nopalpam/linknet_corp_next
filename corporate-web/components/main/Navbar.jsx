...
function transformMenuData(cmsMenus, locale) {
  ...
  const MAX_MEGA_ITEMS_PER_COLUMN = 10;
    ...
    .map((menu) => {
      ...
      if (children.length > 0) {
        ...
        children.forEach((child) => {
          ...
          const sectionOrder = Number.isFinite(Number(child.sectionOrder)) ? Number(child.sectionOrder) : 0;
          const sectionKey = `${sectionOrder}:${title}`;

          if (!sectionsByTitle.has(sectionKey)) {
            sectionsByTitle.set(sectionKey, {
              title,
              sectionOrder,
              items: [],
            });
          ...
          sectionsByTitle.get(sectionKey).items.push({
        ...
        navItem.sections = Array.from(sectionsByTitle.values())
          ...
          .flatMap((section) => {
            if ((section.items?.length || 0) <= MAX_MEGA_ITEMS_PER_COLUMN) {
              return [section];
            }

            const chunks = [];
            for (let index = 0; index < section.items.length; index += MAX_MEGA_ITEMS_PER_COLUMN) {
              chunks.push({
                ...section,
                title: index === 0 ? section.title : '',
                sectionOrder: (section.sectionOrder || 0) + index / MAX_MEGA_ITEMS_PER_COLUMN,
                items: section.items.slice(index, index + MAX_MEGA_ITEMS_PER_COLUMN),
              });
            }
...
function localizeHref(href, locale, defaultLocale) {
  ...
  const localizedSegments = locale === defaultLocale ? segments : [locale, ...segments];
  const localizedPath = `/${localizedSegments.join('/')}`.replace(/\/+$/, '') || '/';
  return `${localizedPath}${suffix}`;
}

function localizeNavItem(item, locale, defaultLocale) {
  return {
    ...item,
    url: localizeHref(item.url, locale, defaultLocale),
    sections: item.sections?.map((section) => ({
      ...section,
      items: section.items?.map((sectionItem) => ({
        ...sectionItem,
        url: localizeHref(sectionItem.url, locale, defaultLocale),
      })),
    })),
  };
}

function localizedSettingValue(value, locale, fallback = '') {
...
function resolveNavbarCtas(settings, locale, defaultLocale) {
  const cta = settings?.navbar?.cta || {};
  const contact = cta.contact || {};
  const consult = cta.consult || {};

  return {
    contact: {
      show: settingEnabled(contact.show ?? contact.visible, true),
      label: localizedSettingValue(contact.label, locale, 'Contact Us'),
      url: localizeHref(contact.url || '/contact-us', locale, defaultLocale),
    },
    consult: {
      show: settingEnabled(consult.show ?? consult.visible, true),
      label: localizedSettingValue(consult.label, locale, 'Consult Now'),
      url: localizeHref(consult.url || '/enterprise-consultation', locale, defaultLocale),
    },
  };
}

export default function Navbar({ menuData, defaultLocale = 'en', settings = null }) {
  const locale = useLocale();
  const pathname = usePathname();
  const router = useRouter();
  const searchParams = useSearchParams();
  const activeDefaultLocale = normalizeLocale(defaultLocale);
  const cmsNavItems = transformMenuData(menuData, locale);
  const navItems = (cmsNavItems?.length ? cmsNavItems : enterpriseNavItems)
    .map((item) => localizeNavItem(item, locale, activeDefaultLocale));
  ...
  const navbarCtas = resolveNavbarCtas(settings, locale, activeDefaultLocale);

  // DEBUG: log data yang diterima Navbar dari server
  if (typeof window !== 'undefined') {
    if (!menuData || (Array.isArray(menuData) && menuData.length === 0)) {
      console.warn('[Navbar] menuData kosong atau tidak diterima dari server \u2014 menggunakan menu hardcoded fallback');
    } else {
      console.log(`[Navbar] menuData diterima: ${menuData.length} item`);
    }
    if (!branding.logo_dark && !branding.logo) {
      console.warn('[Navbar] branding.logo tidak ditemukan di settings \u2014 menggunakan logo fallback. settings.general_branding:', settings?.general_branding ?? null);
    } else {
      console.log('[Navbar] logoSrc:', logoSrc);
    }
    if (!cmsNavItems) {
      console.warn('[Navbar] transformMenuData() mengembalikan null \u2014 menggunakan enterpriseNavItems sebagai fallback');
    }
  }
  ...
  const popularSearches = [
    ...
    ...item,
    url: localizeHref(item.url, locale, activeDefaultLocale),
  }));
  ...
  const switchLocale = (newLocale) => {
    ...
    const nextSegments = newLocale === activeDefaultLocale ? segments : [newLocale, ...segments];
    const nextPath = `/${nextSegments.join('/')}`.replace(/\/+$/, '') || '/';
    const queryString = searchParams.toString();
    document.cookie = `NEXT_LOCALE=${newLocale}; path=/; max-age=31536000; samesite=lax`;
    router.replace(queryString ? `${nextPath}?${queryString}` : nextPath, { scroll: false });
    setActiveDropdown(null);
    setIsDrawerOpen(false);
  };

  const navigateToCta = (href) => {
    if (!href) return;
    setActiveDropdown(null);
    setIsDrawerOpen(false);
          ...
          >
            ...
            <div className="lnNavbar__desktop-actions hidden xl:flex items-center gap-4">
              ...
              <div className="lnNavbar__action-group flex items-center gap-4">
                ...
                {(navbarCtas.contact.show || navbarCtas.consult.show) && (
                  <div className='lnNavbar__cta-group flex gap-2'>
                    {navbarCtas.contact.show && (
                      <Button type="button" variant="secondary-outline" size="md" className="lnNavbar__cta-secondary" onClick={() => navigateToCta(navbarCtas.contact.url)}>
                        {navbarCtas.contact.label}
                      </Button>
                    )}
                    {navbarCtas.consult.show && (
                      <Button type="button" variant="primary" size="md" className="lnNavbar__cta-primary" onClick={() => navigateToCta(navbarCtas.consult.url)}>
                        {navbarCtas.consult.label}
                      </Button>
                    )}
                  </div>
                )}
      ...
      >
        <div ref={mobileDrawerRef} className="lnNavbar__drawer-inner relative w-full h-full overflow-hidden flex flex-col">
          ...
          <div className="lnNavbar__drawer-header flex items-center justify-between px-[16px] py-3 bg-white z-10">
             <img src={logoSrc} alt="Link Net" className="h-8 w-auto" />
                   ...
                   <div className="flex gap-3 w-full">
                      {navbarCtas.contact.show && (
                        <Button
                           ...
                           onClick={() => navigateToCta(navbarCtas.contact.url)}
                        >
                          {navbarCtas.contact.label}
                        </Button>
                      )}
                      {navbarCtas.consult.show && (
                        <Button type="button" variant="primary" size="md" className="lnNavbar__mobile-footer-item font-bold w-full justify-center !rounded-full" onClick={() => navigateToCta(navbarCtas.consult.url)}>
                          {navbarCtas.consult.label}
                        </Button>
                      )}
                   </div>
