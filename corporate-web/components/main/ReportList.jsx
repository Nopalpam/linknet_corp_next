...
function normalizeReportItem(report, type = {}, section = {}) {
  ...
  return {
    ...
    subDescription: firstValue(report, ['subDescription', 'sub_description', 'description']),
    ...
    downloadUrl: firstValue(report, ['pdfFile', 'pdf_file', 'downloadUrl', 'download_url', 'fileUrl', 'file_url'], '#'),
  };
}
...
function sortGroupsByReference(groups, sections = []) {
  if (!Array.isArray(groups) || groups.length === 0 || !Array.isArray(sections) || sections.length === 0) {
    return groups;
  }
  ...
  return [...groups].sort((a, b) => {
    const aPosition = sectionOrderMap.get(String(a.id)) ?? getSortPosition(a.position ?? a.sort_order ?? a.sortOrder);
    const bPosition = sectionOrderMap.get(String(b.id)) ?? getSortPosition(b.position ?? b.sort_order ?? b.sortOrder);
...
function transformMainData(mainData) {
  if (Array.isArray(mainData?.groups)) return sortGroupsByReference(normalizeReportGroups(mainData.groups), mainData.sections);
  ...
  return sortGroupsByReference(normalizeReportGroups(groups), mainData.sections);
}
...
export default function ReportListPage({
  ...
  const source = useMemo(() => cmsData || {}, [cmsData]);
  ...
  const GROUPS_PER_PAGE = SECTIONS_PER_PAGE;
  ...
  const totalPages = effectiveShowPagination
    ? (layout === 'grid'
  ...
  const paginatedGroups = effectiveShowPagination ? filteredGroups.slice(startIndex, startIndex + GROUPS_PER_PAGE) : filteredGroups;
  const gridItems = effectiveShowPagination ? filteredFlatItems.slice(startIndex, startIndex + GRID_ITEMS_PER_PAGE) : filteredFlatItems;
  ...
  return (
    ...
    >
      <div className="container">
        ...
        {/* PAGINATION CONTROLS */}
        {/* ========================================= */}
        {effectiveShowPagination && totalPages > 1 && (
