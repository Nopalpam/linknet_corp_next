...
function hasVisibleCta(cta) {
  if (!cta || typeof cta !== 'object') return false;
  return Boolean(resolveCtaLabel(cta.label ?? cta.text ?? cta.button_text ?? cta.cta_text).trim());
}
      ...
      <div className="container mx-auto px-4 md:px-0">
        ...
        <div className="flex flex-col">
          {items.map((item, index) => {
            const ctaList = Array.isArray(item.ctaList) ? item.ctaList.filter(hasVisibleCta) : [];
            ...
            return (
              ...
              <div className="md:col-span-8 lg:col-span-9">
                ...
                {item.contents && (
                  <SafeHtml
                    className="rich-text-content text-body-b4 leading-relaxed"
                    html={item.contents}
                  />
                )}
