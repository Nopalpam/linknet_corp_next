...
export default function Hero({
  ...
  return (
    // Outer spacing follows the page background instead of forcing a white strip.
    <div ref={containerRef} className={`p-2 pt-0 ${configClassName} ${className}`}>
                ...
                <div className="max-w-full md:max-w-[560px] flex flex-col items-start gap-2 md:gap-2">
                    ...
                    {resolvedTitle && (
                        <Tag className={`text-headline-h4 md:text-headline-h3 font-bold tracking-tight drop-shadow-sm ${isDark ? 'text-white text-shadow-md' : 'text-black'}`}>
                            <SplitText
                                text={typeof resolvedTitle === 'string' ? normalizeTitleLineBreaks(resolvedTitle) : ''}
                                delay={240}
                                duration={0.5}
                                ease="power3.out"
                                splitType="lines"
