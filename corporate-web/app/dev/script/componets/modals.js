...
(() => {
    ...
    const buildPlainTextBio = (raw = "") => {
        const fragment = document.createDocumentFragment();
        const normalized = raw.replace(/\r\n/g, "\n").replace(/\s*\/n\s*/gi, "\n").trim();

        if (!normalized) return fragment;

        normalized.split(/\n{2,}/g).forEach((paragraphText) => {
    ...
    const sanitizeHTML = (raw = "") => {
        const html = formatParagraphs(raw);
    ...
    const render = (idx) => {
        ...
        els.bio.replaceChildren(sanitizeHTML(d.bio));
