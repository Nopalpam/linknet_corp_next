...
import ClosingSentence from "@/components/main/ClosingSentence";
...
import { getApiDebugSnapshot, isApiDebugEnabled } from "@/lib/apiBaseUrl";
import {
  DEFAULT_OG_IMAGE,
...
export default async function RootLayout({ children, params }) {
  ...
  if (apiDebugEnabled) {
    console.log('[Layout] Fetching CMS data', getApiDebugSnapshot('server'));
  }
  ...
  if (apiDebugEnabled) {
    console.log(`[Layout] CMS data result - menuData: ${menuData.length} items, footerMenus: ${footerMenus.length} sections, publicSettings keys: ${Object.keys(publicSettings).length}`);
    if (menuData.length === 0) console.warn('[Layout] menuData kosong - navbar akan tampil fallback hardcoded');
    if (Object.keys(publicSettings).length === 0) console.warn('[Layout] publicSettings kosong - logo, branding, dan settings lainnya akan tampil fallback');
    else console.log('[Layout] publicSettings.general_branding:', JSON.stringify(publicSettings.general_branding ?? null));
  }
  ...
  const cookiesData = {
    ...
    title: localizedValue(cookies.title, locale, locale === 'id' ? 'Kami menggunakan cookie' : 'We use cookies'),
    ...
    acceptLabel: localizedValue(cookies.accept_label, locale, locale === 'id' ? 'Terima' : 'Accept'),
    savingLabel: locale === 'id' ? 'Menyimpan...' : 'Saving...',
    moreInfoLabel: localizedValue(cookies.more_info?.title, locale, locale === 'id' ? 'Info Selengkapnya' : 'More Info'),
    ...
    <html lang={locale}>
      ...
      <body>
        ...
        <NextIntlClientProvider messages={messages} locale={locale}>
          <VisitorTracker />
          <div id="site-navbar" className="contents">
          ...
          <div id="site-footer" className="contents">
            <Footer cmsFooterData={cmsFooterData} />
          </div>
