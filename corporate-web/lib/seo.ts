...
function resolveFallbacks(publicSettings: Record<string, any>, locale: string) {
  ...
  const title = localizedValue(seo.meta_title, locale, siteTitle);
  ...
  return { title, siteTitle, description, keywords, image };
}
...
export function buildBasicMetadata({
  ...
  const safeTitle = compactText(title) || fallbacks.title || DEFAULT_SITE_TITLE;
...
export function buildCmsPageMetadata({
  ...
  const defaultSeoTitle = localizedValue(publicSettings.seo?.meta_title, locale);
  const title = metaTitle || (isHomePath ? defaultSeoTitle : '') || fallbackTitle;
