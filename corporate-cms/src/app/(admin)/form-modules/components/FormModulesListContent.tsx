...
import FormModulesTable from "./FormModulesTable";

interface FormModulesListContentProps {
  /** When provided, locks the BU filter to this value and hides the BU dropdown */
...
export default function FormModulesListContent({
  fixedBusinessUnit,
  pageTitle,
}: FormModulesListContentProps) {
  ...
  const fetchModules = useCallback(async () => {
    try {
      ...
      const res = await formModuleService.listFormModules({
        ...
        businessUnit: fixedBusinessUnit || undefined,
  ...
  }, [currentPage, searchQuery, fixedBusinessUnit, filterCategory, filterStatus, toast]);
  ...
  const handleToggleStatus = async (mod: FormModule) => {
    ...
    try {
      ...
      toast.success(
        `"${mod.name}" moved to ${next.charAt(0) + next.slice(1).toLowerCase()}`
      );
      await fetchModules();
    } catch (err: any) {
