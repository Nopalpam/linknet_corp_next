...
type FileManagerErrorView = ReturnType<typeof describeFileManagerError>;
type ClipboardItem = {
  type: "file" | "folder";
  id: string;
  name: string;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {error.source && <span className="rounded-full bg-white/70 px-2 py-1 dark:bg-black/20">source: {error.source}</span>}
            {error.status !== undefined && <span className="rounded-full bg-white/70 px-2 py-1 dark:bg-black/20">status: {error.status}</span>}
            {error.code && <span className="rounded-full bg-white/70 px-2 py-1 dark:bg-black/20">code: {error.code}</span>}
            {error.requestId && <span className="rounded-full bg-white/70 px-2 py-1 dark:bg-black/20">request: {error.requestId}</span>}
          </div>
          {error.likelyCause && <p className="text-xs text-red-700 dark:text-red-200/80">{error.likelyCause}</p>}
        </div>
        {onRetry && (
          <button
            type="button"
            onClick={onRetry}
            className="w-fit rounded-lg bg-red-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-red-700"
          >
            Retry
          </button>
        )}
...
type FolderTreeItemProps = {
  ...
  onClipboard: (folder: MediaFolder, action: "copy" | "move") => void;
};
    ...
    <div className="space-y-1">
      <div className="group flex items-center gap-2">
        <button
          ...
          className="rounded px-1.5 py-1 text-[10px] font-medium text-gray-400 opacity-0 transition group-hover:opacity-100 hover:bg-gray-100 hover:text-brand-600 dark:hover:bg-gray-700"
          title="Rename folder"
        >
          R
        </button>
        <button
          type="button"
          onClick={() => onClipboard(folder, "copy")}
          className="rounded px-1.5 py-1 text-[10px] font-medium text-gray-400 opacity-0 transition group-hover:opacity-100 hover:bg-gray-100 hover:text-brand-600 dark:hover:bg-gray-700"
          title="Copy folder"
        >
          C
        ...
        <button
          type="button"
          onClick={() => onClipboard(folder, "move")}
          className="rounded px-1.5 py-1 text-[10px] font-medium text-gray-400 opacity-0 transition group-hover:opacity-100 hover:bg-gray-100 hover:text-amber-600 dark:hover:bg-gray-700"
          title="Cut folder"
        >
          X
        </button>
...
export default function FileManagerPage() {
  ...
  const [fileError, setFileError] = useState<FileManagerErrorView | null>(null);
  const [folderError, setFolderError] = useState<FileManagerErrorView | null>(null);
  const [uploadError, setUploadError] = useState<FileManagerErrorView | null>(null);
  const [storageSummary, setStorageSummary] = useState<FileListResponse["data"]["storage"] | null>(null);
  const [debugInfo, setDebugInfo] = useState<FileManagerDebugResponse | null>(null);
  const [debugError, setDebugError] = useState<FileManagerErrorView | null>(null);
  ...
  const fetchFolders = useCallback(async () => {
    try {
      ...
      const error = describeFileManagerError(err);
  ...
  useEffect(() => {
    ...
    fileManagerService.getDebugInfo()
      .then((response) => {
        if (isMounted) setDebugInfo(response);
      })
      .catch((err: unknown) => {
        if (isMounted) setDebugError(describeFileManagerError(err));
      })
      .finally(() => {
        if (isMounted) setDebugLoading(false);
      });
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (fileRejections) => {
      ...
      const message = firstError?.message || "Upload rejected by CMS before request was sent";
      const error: FileManagerErrorView = {
        message,
        source: "validation",
        code: firstError?.code || "CMS_UPLOAD_REJECTED",
        likelyCause: "CMS-side upload constraints rejected the selected file before corporate-be or corporate-fm was called.",
      };
      setUploadError(error);
      toast.error(message);
    },
  ...
  const handleRenameFolder = useCallback(async (folder: MediaFolder) => {
    ...
    setFolderError(null);
    try {
      const response = await fileManagerService.renameFolder(folder.id, name);
      toast.success(response.message || "Folder renamed successfully", 3000);
      await Promise.all([fetchFolders(), fetchFiles(1)]);
    } catch (err: unknown) {
      const error = describeFileManagerError(err);
      setFolderError(error);
      toast.error(error.message);
  ...
  }, [fetchFiles, fetchFolders, toast]);

  const handleRenameFile = useCallback(async (file: FileItem) => {
    const name = window.prompt("New file name", file.originalName)?.trim();
    if (!name || name === file.originalName) return;

    setMutating(true);
    setFileError(null);
    try {
      const response = await fileManagerService.renameFile(file.id, name);
      toast.success(response.message || "File renamed successfully", 3000);
      setShowDetail(false);
  ...
  const putOnClipboard = useCallback((item: Omit<ClipboardItem, "action">, action: "copy" | "move") => {
    setClipboard({ ...item, action });
    toast.success(`${item.name} ready to ${action === "copy" ? "copy" : "move"}`, 2500);
  }, [toast]);
  ...
  const handlePaste = useCallback(async () => {
    ...
    setFolderError(null);
    try {
      const response = clipboard.type === "file"
        ? await fileManagerService.transferFile(clipboard.id, clipboard.action, selectedFolderId || undefined)
        : await fileManagerService.transferFolder(clipboard.id, clipboard.action, selectedFolderId || undefined);
      toast.success(response.message || "Item pasted successfully", 3000);
      if (clipboard.action === "move") setClipboard(null);
      await Promise.all([fetchFolders(), fetchFiles(1)]);
    } catch (err: unknown) {
      const error = describeFileManagerError(err);
      if (clipboard.type === "folder") setFolderError(error);
      else setFileError(error);
      toast.error(error.message);
    } finally {
      setMutating(false);
    }
  }, [clipboard, fetchFiles, fetchFolders, selectedFolderId, toast]);
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="border-b border-gray-200 p-6 dark:border-gray-800">
          <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <span className="inline-flex w-fit rounded-full bg-brand-50 px-3 py-1 text-xs font-medium text-brand-700 dark:bg-brand-900/20 dark:text-brand-300">
              {selectedFolder ? `Folder: ${selectedFolder.path}` : "Folder: S3 root"}
            </span>
          </div>

          {showDebug && (
            <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900 dark:border-amber-900/50 dark:bg-amber-900/20 dark:text-amber-100">
              <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="font-semibold">Development debug trace</p>
                  <p className="text-amber-800 dark:text-amber-100/80">
                    CMS to corporate-be to corporate-fm to S3
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    ...
                    fileManagerService.getDebugInfo()
                      .then(setDebugInfo)
                      .catch((err: unknown) => setDebugError(describeFileManagerError(err)))
                      .finally(() => setDebugLoading(false));
                  }}
                  className="w-fit rounded-lg bg-amber-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-amber-700"
                >
                  {debugLoading ? "Checking..." : "Recheck"}
                </button>
              </div>
              {debugError ? (
                <ErrorState title="Debug endpoint failed" error={debugError} />
              ) : (
                <div className="grid gap-2 md:grid-cols-4">
                  <div className="rounded border border-amber-200 bg-white/70 p-2 dark:border-amber-900/40 dark:bg-black/20">
                    <p className="font-medium">Gateway request</p>
                    <p className="break-all">{debugInfo?.requestId || "pending"}</p>
                  </div>
                  <div className="rounded border border-amber-200 bg-white/70 p-2 dark:border-amber-900/40 dark:bg-black/20">
                    <p className="font-medium">S3 prefix</p>
                    <p className="break-all">{storageSummary?.prefix || "checking root prefix"}</p>
                  </div>
                  <div className="rounded border border-amber-200 bg-white/70 p-2 dark:border-amber-900/40 dark:bg-black/20">
                    <p className="font-medium">S3 objects seen</p>
                    <p>{storageSummary ? storageSummary.objectCountFromS3List : "pending"}</p>
                  </div>
                  <div className="rounded border border-amber-200 bg-white/70 p-2 dark:border-amber-900/40 dark:bg-black/20">
                    <p className="font-medium">S3-only objects</p>
                    <p>{storageSummary ? storageSummary.s3OnlyObjectCount || 0 : "pending"}</p>
                  </div>
                </div>
              )}
              {debugInfo && (
                <details className="mt-3">
                  <summary className="cursor-pointer font-medium">Raw safe debug payload</summary>
                  <pre className="mt-2 max-h-72 overflow-auto rounded bg-white/80 p-3 text-[11px] dark:bg-black/30">
          ...
          {uploadError && (
            <div className="mt-4">
              <ErrorState title="Upload failed" error={uploadError} />
            </div>
          )}
        </div>
        ...
        <div className="grid lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="border-b border-gray-200 p-4 dark:border-gray-800 lg:border-b-0 lg:border-r">
            ...
            {folderError && (
              <div className="mb-3">
                <ErrorState title="Folder request failed" error={folderError} onRetry={() => void fetchFolders()} />
              </div>
            )}
            ...
            <div className="max-h-[560px] space-y-1 overflow-y-auto pr-1">
              {foldersLoading ? (
                ...
                <p className="rounded-lg border border-dashed border-gray-300 px-3 py-4 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
                  No folders yet. Create the first folder or upload directly into the S3 root.
                </p>
              ) : (
                folders.map((folder) => (
                  <FolderTreeItem
                    ...
                    onRename={(targetFolder) => void handleRenameFolder(targetFolder)}
                    onClipboard={(targetFolder, action) => putOnClipboard({
                      type: "folder",
                      id: targetFolder.id,
          ...
          <section className="min-w-0">
            <div className="border-b border-gray-200 p-4 dark:border-gray-800">
              <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {clipboard && (
                    <>
                      <span className="max-w-52 truncate rounded-full bg-amber-50 px-3 py-1 text-xs text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                        {clipboard.action === "copy" ? "Copy" : "Cut"}: {clipboard.name}
                      </span>
                      <button
                        type="button"
                      ...
                      >
                        {mutating ? "Working..." : `Paste to ${selectedFolder?.name || "root"}`}
                      </button>
                      <button
                        type="button"
                        onClick={() => setClipboard(null)}
                        disabled={mutating}
                        className="rounded-lg border border-gray-300 px-2 py-1.5 text-xs text-gray-500 dark:border-gray-600 dark:text-gray-300"
                      >
                        Clear
                      </button>
                    </>
            ...
            <div className="p-4">
              {fileError ? (
                <ErrorState title="File list request failed" error={fileError} onRetry={() => void fetchFiles(currentPage)} />
              ) : loading ? (
          ...
          <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800">
            ...
            <div className="mb-6 space-y-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-800 dark:text-gray-200">{selectedFile.folder?.path || "S3 root"}</p>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <>
                  <button
                    ...
                    disabled={mutating}
                    className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
                  >
                    Rename
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      putOnClipboard({ type: "file", id: selectedFile.id, name: selectedFile.originalName }, "copy");
                      setShowDetail(false);
                      setSelectedFile(null);
                    }}
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      putOnClipboard({ type: "file", id: selectedFile.id, name: selectedFile.originalName }, "move");
                      setShowDetail(false);
                      setSelectedFile(null);
                    }}
