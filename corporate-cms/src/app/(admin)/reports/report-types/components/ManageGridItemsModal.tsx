...
interface ManageGridItemsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ManageGridItemsModal({
  isOpen,
  onClose,
  reportType,
  onUpdate,
}: ManageGridItemsModalProps) {
  ...
  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const response = await reportService.getReportTypeGridItems(reportType.id);
      setItems(response.data || []);
    } catch (err: any) {
      toast.error(err.message || "Failed to fetch grid items");
    } finally {
      setLoading(false);
    }
  }, [reportType.id, toast]);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      setSaving(true);
      await reportService.updateGridItemsOrder(reportType.id, updates);
      toast.success("Grid items order updated");
      onUpdate();
    } catch (err: any) {
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Drag and drop to reorder grid items for{" "}
            <span className="font-semibold">{reportType.name}</span>
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-gray-500 dark:text-gray-400">
            <p>No grid items found for this report type.</p>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-center text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Type</th>
