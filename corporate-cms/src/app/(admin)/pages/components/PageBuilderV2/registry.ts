...
function withCommon(data: Record<string, any>): Record<string, any> {
  ...
  return {
    ...
    ...(Array.isArray(ctaSource) ? { ctaList: normalizedCtaList } : {}),
    ...rest,
    config: {
...
const SYNCED_MAIN_COMPONENTS: ComponentRegistryEntry[] = [
  ...
  syncedMainComponent('tv_channel_list', 'TV Channel List', '@/components/main/TVChannelList', { icon: 'FaPlayCircle' }),
...
export const STATIC_COMPONENT_TYPES: ComponentRegistryEntry[] = [
  ...
  },
  {
    ...
    defaultData: withCommon({
      introData: {
        title: { en: 'Ready?', id: 'Siap?' },
        description: { en: 'Contact us.', id: 'Hubungi kami.' },
        align: 'center',
      },
      ctaList: [{ text: { en: 'Get Started', id: 'Mulai' }, href: '#', target: '_self', variant: 'secondary-outline', size: 'lg' }],
      video_url: '',
      bg_image: '',
    }),
  },
