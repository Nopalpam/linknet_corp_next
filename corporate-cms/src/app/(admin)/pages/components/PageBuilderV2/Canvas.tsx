...
function resolvePreviewField(source: PreviewSettings | undefined, field: string): string {
  ...
  const isLocalizedValue = value !== null
    && typeof value === 'object'
    && !Array.isArray(value)
    && ('en' in value || 'id' in value);
  const localizedValue = typeof value === 'string' || isLocalizedValue
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900/60">
      ...
      <div className="space-y-4 p-4">
        {presentation.items.length === 0 && (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-slate-50 px-4 py-6 text-center text-xs font-medium text-gray-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400">
            No business tabs configured yet
          </div>
        )}
