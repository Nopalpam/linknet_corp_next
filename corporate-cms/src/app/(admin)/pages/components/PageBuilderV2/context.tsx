...
function normalizeSectionIntro(settings: Record<string, any>) {
  const introSource = settings.introData || settings.sectionIntro || settings.intro;
  const explicitIntro = introSource && typeof introSource === 'object' && !Array.isArray(introSource)
    ? introSource
    : {};
  const hasExplicitIntroField = (field: string) => Object.prototype.hasOwnProperty.call(explicitIntro, field);
  const introData = {
  ...
  if (!hasExplicitIntroField('title') && !hasRenderableValue(introData.title) && hasRenderableValue(settings.title)) {
    introData.title = settings.title;
  }
  if (!hasExplicitIntroField('description') && !hasRenderableValue(introData.description) && hasRenderableValue(settings.description)) {
