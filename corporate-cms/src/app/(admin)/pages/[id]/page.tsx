...
const FALLBACK_PREVIEW_BASE_URL = "https://dev.linknet.co.id";
...
function getSafePreviewBaseUrl(value: string) {
  try {
    const url = new URL(value);
    if (url.protocol === "https:" || (url.protocol === "http:" && LOCAL_PREVIEW_HOSTS.has(url.hostname))) {
      return trimTrailingSlashes(url.origin);
    }
  } catch {
    // Fall through to the known HTTPS default.
  }

  return FALLBACK_PREVIEW_BASE_URL;
}

function getSafePreviewPath(template: string, slug: string) {
  const trimmedTemplate = template.trim();
  const safeTemplate = trimmedTemplate.startsWith("/") && !trimmedTemplate.startsWith("//")
    ? trimmedTemplate
    : "/pages/{slug}";

  return safeTemplate.split("{slug}").join(encodeSlugPath(slug));
}
...
export default function EditPagePage() {
  ...
  useEffect(() => {
    const fetchPreviewSettings = async () => {
      try {
        ...
        setPreviewBaseUrl(process.env.NEXT_PUBLIC_SITE_URL || "https://dev-be.lncorp.local");
      }
    };
  ...
  const getPagePreviewUrl = (slug: string) => {
    const base = getSafePreviewBaseUrl(previewBaseUrl || process.env.NEXT_PUBLIC_SITE_URL || FALLBACK_PREVIEW_BASE_URL);
    const path = getSafePreviewPath(previewPathTemplate || "/pages/{slug}", slug);
    return `${base}${path}`;
  };
            ...
            <div className="space-y-5 p-5">
              ...
              <InfoBlock title="Setting">
                <ToggleRow
                  ...
                  onChange={(checked) => setFormData((current) => ({ ...current, showNavbar: checked }))}
                />
                <ToggleRow
                  label="Footer"
                  checked={formData.showFooter ?? true}
                  onChange={(checked) => setFormData((current) => ({ ...current, showFooter: checked }))}
                />
              </InfoBlock>
