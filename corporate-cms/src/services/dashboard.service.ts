...
class DashboardService extends BaseService {
  ...
  async getContentOverview(): Promise<ContentOverviewData> {
    ...
    return {
      pages: {
        total: Number(data.pages?.total) || 0,
        published: Number(data.pages?.published) || 0,
        draft: Number(data.pages?.draft) || 0,
      },
      menus: { total: Number(data.menus?.total) || 0 },
      news: {
        total: Number(data.news?.total) || 0,
        published: Number(data.news?.published) || 0,
        draft: Number(data.news?.draft) || 0,
      },
      files: { total: Number(data.files?.total) || 0 },
    };
  }
