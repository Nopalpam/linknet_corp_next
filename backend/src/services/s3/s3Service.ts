...
class S3Service {
  ...
  async uploadFile(
    ...
    try {
      ...
      logger.info(`[S3] Uploaded file: ${key} (${buffer.length} bytes)`);
  ...
  async uploadLargeFile(
    ...
    try {
      ...
      logger.info(`[S3] Multipart upload completed: ${key} (${buffer.length} bytes)`);
  ...
  async generatePresignedUploadUrl(
    ...
    try {
      ...
      logger.info(`[S3] Generated presigned upload URL for: ${key} (expires in ${expiresIn}s)`);
  ...
  async fileExists(key: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error(`[S3] fileExists check failed for ${key}:`, error);
      return false;
    }
  }
  ...
  async getFileMetadata(key: string): Promise<S3FileInfo | null> {
    ...
    try {
      ...
      logger.error(`[S3] getFileMetadata failed for ${key}:`, error);
      return null;
    }
  }
