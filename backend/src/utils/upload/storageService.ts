...
class StorageService {
  ...
  async uploadFile(
    ...
    logger.info(`[Storage] Uploading via '${driver}' driver — ${originalName} (${buffer.length} bytes)`);
  ...
  async deleteFile(key: string): Promise<StorageDeleteResult> {
    ...
    logger.info(`[Storage] Deleting via '${driver}' driver — ${key}`);
    ...
    switch (driver) {
      case 's3':
        try {
          ...
          logger.error(`[Storage:s3] Delete failed for ${key}:`, error);
          return { success: false, key };
        }
      ...
      case 'azure':
        try {
          ...
          logger.error(`[Storage:azure] Delete failed for ${key}:`, error);
          return { success: false, key };
        }
