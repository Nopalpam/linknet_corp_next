...
(() => {
  ...
  function show(page){
    if (!page) return; // Add null check
    Object.values(pages).forEach(p => {
      if (p) p.classList.remove('is-active'); // Add null check for each page
    });
...
document.querySelectorAll('#offcanvasLanguage .list-group-item').forEach(item => {
  item.addEventListener('click', () => {
    ...
    const langCheck = item.querySelector('.lang-check');
    if (langCheck) {
      langCheck.classList.remove('d-none');
      // contoh: simpan pilihan
      const lang = langCheck.dataset.lang;
