...
export default function AnnouncementSectionsPage() {
  ...
  const [selectedSection, setSelectedSection] = useState<AnnouncementSection | null>(null);
  const [formMode, setFormMode] = useState<"create" | "edit">("create");

  // Fetch announcement types for dropdown
  ...
  const fetchSections = useCallback(async () => {
    try {
      setLoading(true);
      const response = await announcementService.getAnnouncementSections({
  ...
  const handleEdit = (item: AnnouncementSection) => {
    setFormMode("edit");
    setSelectedSection(item);
    setIsFormModalOpen(true);
  };

  const handleDelete = (item: AnnouncementSection) => {
    setSelectedSection(item);
    setIsDeleteModalOpen(true);
  };

  const handleToggleStatus = async (item: AnnouncementSection) => {
    try {
      const response = await announcementService.toggleAnnouncementSectionStatus(item.id);
  ...
  const handleManageItems = (item: AnnouncementSection) => {
    setSelectedSection(item);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await announcementService.deleteAnnouncementSection(selectedSection.id);
  ...
  const handleBulkDelete = async () => {
    try {
      await announcementService.deleteMultipleAnnouncementSections(selectedIds);
  ...
  // Columns
  const columns: TableColumn<AnnouncementSection>[] = [
    {
      key: "title",
      label: "Title",
      sortable: true,
  ...
  return (
    <div>
      <PageBreadCrumb pageTitle="Announcement Sections" />
      ...
      <div className="rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
        <DataTable
          ...
          emptyMessage="No announcement sections found"
          actions={(item) => (
            <>
              {/* Manage Items */}
              <button
      ...
      <BulkDeleteModal
        ...
        itemName="announcement sections"
      />
