...
export default function ManageSectionItemsModal({
  ...
  const fetchItems = useCallback(async () => {
    try {
      setLoading(true);
      const response = await announcementService.getAnnouncementSectionItems(section.id);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      setSaving(true);
      await announcementService.updateSectionItemsOrder(section.id, updates);
  ...
  const handleToggleStatus = async (item: AnnouncementItem) => {
    try {
      await announcementService.toggleAnnouncementItemStatus(item.id);
      ...
      <div className="p-6">
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            {section.announcementYear && ` (${section.announcementYear})`}
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm mt-1">Create items from the Announcement Items page.</p>
          </div>
        ) : (
          <DndContext
            sensors={sensors}
