...
export default function HighlightingRealInitiatives({ 
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      const gsapElements = gsap.utils.toArray('.gsap-initiative-item');
  ...
  const { introData, items, partnerText, partnerLogos, ctaList } = sectionData;
  ...
  return (
    ...
    <section ref={containerRef} className={`py-16 md:py-24 bg-light-2 rounded-[40px] overflow-hidden ${className}`}>
      ...
      <div className="container mx-auto px-4 md:px-0">
        ...
        {introData && (
          ...
          <div className="mb-10 gsap-initiative-item">
            <Intro 
              as={introData.as || "h2"}
            ...
            >
              {items.map((item, index) => (
                <SwiperSlide key={item.id || index} className="h-auto">
                  ...
                  <div className="gsap-initiative-item h-full">
        ...
        <div className="flex flex-col items-center justify-center text-center">
          ...
          {partnerText && (
            // Tambahkan class gsap-initiative-item
            <p className="gsap-initiative-item text-body-b4 text-neutral-400 mb-8 w-[80%] mx-auto">
              {partnerText}
            </p>
          )}
          ...
          {duplicatedLogos.length > 0 && (
            // Tambahkan class gsap-initiative-item
            <div className="gsap-initiative-item w-full overflow-hidden mb-12 relative [mask-image:_linear-gradient(to_right,transparent_0,_black_100px,_black_calc(100%-100px),transparent_100%)]">
          ...
          {ctaList && ctaList.length > 0 && (
            <div className="mt-4 flex flex-wrap justify-center gap-4">
              {ctaList.map((cta, index) => (
                // Tambahkan class gsap-initiative-item untuk setiap tombol
                <div key={index} className="gsap-initiative-item">
                  <Button 
                    variant={cta.variant || 'secondary-outline'} 
                    size={cta.size || 'lg'} 
                    href={cta.href}
                    className="rounded-full px-8 bg-white border-neutral-200"
                  >
                    {cta.text}
                  </Button>
                </div>
